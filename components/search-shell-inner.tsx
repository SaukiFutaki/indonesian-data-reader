...
import { SearchTabs, ResultCard, SegmentedInput, type SearchTab, type Segment } from "@/components/ui";
import { NikFormat, NikResultCard } from "@/components/nik";
import { KodeposFormat, KodeposResultCard } from "@/components/kodepos";
import { PlatFormat } from "@/components/plat";
import { NpsnFormat } from "@/components/npsn";
...
const descriptors: Record<SearchTab, { title: string; hint: string }> = {
  nik: {
    title: "NIK Reader",
...
const segmentConfigs: Record<SearchTab, Segment[]> = {
  nik: [
...
export function SearchShellInner() {
  const [tab, setTab] = useState<SearchTab>("nik");
  ...
  const segments = segmentConfigs[tab];
  const totalLength = useMemo(
    () => segments.reduce((sum, s) => sum + s.maxLength, 0),
    [segments]
  );

  const nikEnabled = tab === "nik" && debounced.length >= 2;
  const kodeposEnabled = tab === "kodepos" && debounced.length === 5;
  const npsnEnabled = tab === "npsn" && debounced.length === 8;
  const platEnabled = tab === "plat" && debounced.length >= 1;
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-kumo-subtle mt-1 max-w-lg">{d.hint}</p>
      </div>
      ...
      <div className="min-h-[12rem]">
        {loading && (
          <p className="text-sm text-kumo-subtle animate-pulse">Mencari...</p>
        )}

        {error && !loading && (
          <LayerCard className="border-red-200 bg-red-50 dark:bg-red-950/20">
            <p className="text-sm text-red-600 dark:text-red-400">{error}</p>
          </LayerCard>
        )}

        {tab === "nik" && query.length >= 2 && result && !error && (
          <NikResultCard data={result as any} segments={segments} value={query} />
        )}

        {tab !== "nik" && result && !error && tab !== "kodepos" && (
          <ResultCard data={result} />
        )}

        {tab === "kodepos" && kodeposResults.length > 0 && !error && (
        ...
        {((tab === "nik" && query.length < 2) ||
          (tab === "kodepos" && query.length < 5) ||
          (tab === "plat" && query.length < 1) ||
          (tab === "npsn" && query.length < 8)) &&
          !result &&
          !loading && (
            <>
              {tab === "nik" && <NikFormat />}
              {tab === "kodepos" && <KodeposFormat />}
              {tab === "plat" && <PlatFormat />}
              {tab === "npsn" && <NpsnFormat />}
            </>
          )}
